...
- ✅ **Electron 9.1.1 → 44.4.5 升级**（2026-09-24，Chromium 152 / Node 24）
  ...
  - 导入弹窗/目录列表暗色适配（2026-09-24）：① 「导入在线书页」说明区两行 `<h4>`（antd reset `h1-h6{color:rgba(0,0,0,.85)}`，近黑看不清）→ 弹窗内 h1-h6 统一 #999999，`.ant-typography` → #666666；② 「导入本地TXT」`.ant-upload-drag` 默认 #fafafa 纯白 → #161819 + 边框 #26292b，`.ant-upload-text` → #999999、`.ant-upload-hint` → #666666；③ 导入预览章节列表与阅读页目录抽屉（`nz-list`，默认 `color:rgba(0,0,0,.65)` 近黑）→ `.ant-list` 系列 → #666666、split 分隔线 #f0f0f0 → #26292b，`.ant-list .current-chapter-menu-item` 高亮规则加权重申 #999999（防被 `.ant-list .ant-list-item` 同 important 高优先级覆盖）；④ 目录抽屉顶部 `.ant-drawer-header` 默认 `background:#fff` 纯白 → #161819，`.ant-drawer-title` → #999999，`.ant-drawer-close` → #666666；⑤ 弹窗遮罩右侧/底部白条：CDK BlockScrollStrategy 给 html 强制 `overflow-y:scroll`，fixed 遮罩的初始包含块不含滚动条槽（15px），槽位透出 antd 默认 `body{background:#fff}` → 注入 `body[data-color-mode="dark"][data-default-theme="1"]{background:#161819 !important}`（body 背景传播到整个画布）。全部经 CDP 截图逐项验证（含本地起假书站 127.0.0.1:8899 走通 识别→预览→导入→目录抽屉 全流程）
  - **兼容补丁 ⑦（2026-09-24）：`_pouch_config` 污染修复——暗色模式启动变亮色的根因。** 全局深色主题的启动脚本裸调 `indexedDB.open('_pouch_config')`（不带版本号），若库不存在会**创建 v1 空库（无任何对象仓库）**；该脚本在 `<head>` 里先于 Angular/PouchDB 执行，PouchDB 之后拿到这个残废库无法初始化 → 配置读写全部静默失败、COLOR_MODE 丢失 → 启动无 `data-color-mode` 打标 → 亮色主题（用户实际库已确认 `_pouch_config:v1 stores:[]`；原有配置内容此前已不可读）。修复：脚本先 `indexedDB.databases()` 探测存在性，不存在则不打开（不再创建空库）；打开后发现缺 `document-store`/`by-sequence`（被污染的空库）则 `deleteDatabase` 让 PouchDB 重建为 v5。**注意**：修复后首次启动配置库被重建，COLOR_MODE 等设置重置为默认（启动会是亮色一次），在阅读页点一次「亮暗切换」即持久化，之后启动恢复暗色。已在 /tmp 隔离 profile 验证：自愈 → v5 全仓库 → 灯泡持久化 → 重启 200ms 内 dark 打标
